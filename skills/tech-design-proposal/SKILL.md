...
**代码示例语言选择：**
1. 优先使用当前项目技术栈对应的语言（基于上述检测结果）
2. 如果项目没有对应技术栈或技术栈不明确，默认使用 Go 语言
3. 特殊场景可使用其他语言（如 Python 用于数据分析示例）
...
**严格执行顺序：**
1. 使用 `write` 工具生成完整的技术方案文档
2. **确认文件写入成功**（等待 write 工具返回成功）
3. 使用 `bash` 工具执行 `code <文件路径>` 命令打开文件
...
- 在当前 VSCode 窗口以编辑模式打开文件
- 如工作区外使用，提示用户可能需要手动打开文件
